...
description=\
"This script takes a pdb file of a TCR-pMHC complex and determines the crossing angle of TCR binding. This crossing angle describes the orientation at which the TCR binds over the pMHC \
and has typically been used to define the mode of binding. The script requires the user to indicate the MHC class i.e. I or II of the complex structure. A number of files are outputted \
including a txt file which contains the angle (and coordinates of how this angle was generated) as well as a number of png image files which visualise the crossing angle generated using PyMOL"
...
def parse_args():
    ...
    parser.add_argument('--MHCclass', dest = 'MHCclass', type = str, required = True, help ='The PDB file to be analysed')
    parser.add_argument('--Fasta', dest = 'fasta', type = str, required = False, default = None, help ='The optional fasta file that contains information to make it easier to localise the TCR')
    parser.add_argument('--Ray', dest = 'ray',   type = str, required = False, help ='Do you want to render images. Set to False for speed and dev. Default = True', default = True)
...
### Structure aligner to reference ###
...
def findMHCaxisCoords(atomMatrix,MHCclass, MHCachain, MHCbchain):
    if MHCclass == "I":
        ...
        return MHCaxisCoords(MHCaxisI(atomMatrix, MHCachain))
    if MHCclass == "II":
        print 'Establishing MHC II axis...'
        return MHCaxisCoords(MHCaxisII(atomMatrix, MHCachain, MHCbchain))        
...
print('     ~  Running crossingAngle.py v2.0 BETA  ~')
...
initialisePymol()
align2template(fileName, MHCclass)
print "Opening aligned PDB file"
alignedPDBnoMeta=readFile(fileName+"/crossingAngle/"+fileName+"_aligned_noMeta.pdb", "pdb")
 
# Extract all lines from PDB file of orig and aligned PDB files #
 
origallLines=fileToList(origPDB)
alignedAllLines=fileToList(alignedPDBnoMeta)
...
MetaSS=SSParser(origallLines)
workingFile = open(fileName+"/crossingAngle/"+fileName+'_aligned.pdb' , 'w')
outTxtMeta=''
...
workingFile = open(fileName+"/crossingAngle/"+fileName+'_aligned.pdb' , 'r')
allLines=fileToList(workingFile)
workingFile.close()
...
if args.fasta != None:
    ...
    TCRaCys1,TCRaCys2,TCRbCys1,TCRbCys2=cysFromSSBONDWrapper(elseList, atomMatrix, TCRachain, TCRbchain)
...
MHCcoords=findMHCaxisCoords(atomMatrix,MHCclass, MHCachain, MHCbchain)
...
MHCtoTCRB = MHC[0] - TCR[1]
MHCatTCRB = MHC - MHCtoTCRB
 
print "\nMHCatTCRB is..."
print type(MHCatTCRB), "shape = ", MHCtoTCRB.shape
print MHCatTCRB
...
X,Y = np.meshgrid(np.linspace(mn[0]-20, mx[0]+20, 20), np.linspace(mn[1]-20, mx[1]+20, 20)) # min/max +/- 20 seems to capture a good area
print "Calculating plane.."
projectionPlane = calc_plane_bis(MHCdata[:,0], MHCdata[:,1], MHCdata[:,2]) # this creates the coefficients of the plane in form ax + by +cz = d where d = 1
...
Z = (1 - projectionPlane[0]*X - projectionPlane[1]*Y)/projectionPlane[2] # rearrangement of above plane equation for values of Z
print "..Done!\n"
...
MHCAproj2=project_points(MHCatTCRB[0,0],MHCatTCRB[0,1],MHCatTCRB[0,2], projectionPlane[0], projectionPlane[1], projectionPlane[2])
MHCBproj2=project_points(MHCatTCRB[1,0],MHCatTCRB[1,1],MHCatTCRB[1,2], projectionPlane[0], projectionPlane[1], projectionPlane[2])
...
MHCaatTCRBtxt = MHCatTCRB[0].tolist()
MHCbatTCRBtxt = MHCatTCRB[1].tolist()
...
BtoBP = np.asscalar(numpy.linalg.norm(TCR[1]-TCRproj[1]))
AtoAP = np.asscalar(numpy.linalg.norm(TCR[0]-TCRproj[0]))
