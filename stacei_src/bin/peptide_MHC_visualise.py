...
def visualise_omit_MHC_only(pdb, mtz, MHCclass, chains, ray, file_name):
    ...
    executable_names = ["fft", "mapmask", "pdbcur", "refmac5", "mtzdump"]
    executable_paths = []
    ...
    local_template = "data/" + MHCclass + "_template.pdb"
    full_path_template = read_pkg_file(local_template)

    #load that, too
    print(full_path_template)
    pymol.cmd.load(full_path_template)
...
def omit_map(pdb, mtz, MHCclass, chains, ray, file_name):
    ...
    for name in executable_names:
        ...
        executable = read_pkg_file(executable)
        executable_paths.append(executable)

    executable_dict = dict(zip(executable_names, executable_paths))

    # delete chain
    print("Deleting chain for %s" % pdb)
    os.system(executable_dict["pdb_cur"] + " XYZIN " + pdb + " XYZOUT " + file_name + "/pdbs/" + file_name +
    ...
    os.system(executable_dict["refmac5"] + " XYZIN " + file_name + "/pdbs/" + file_name + "_nopeptide.pdb" + " XYZOUT " +
    ...
    os.system(executable_dict["mtzdump"] + " HKLIN " + prefix + "_refmac5_omitmap.mtz END")
    ...
    os.system(executable_dict["fft"] + " HKLIN " + prefix + "_refmac5_omitmap.mtz" + " MAPOUT " + file_name +
              "/electrostatics/" + file_name + "_om.map1.tmp" + " < " + "bin/data/EDMparam1.tmp")
    print("Running mapmask with parameters 1")
    os.system(executable_dict["mapmask"] + " MAPIN " + prefix + "_om.map1.tmp" +
    ...
    os.system(executable_dict["fft"] + " HKLIN " + prefix + "_refmac5_omitmap.mtz"
              + " MAPOUT " + prefix + "_om.map3.tmp" + " < " + " bin/data/EDMparam3.tmp")
    print("Running mapmask with parameters 2")
    os.system(executable_dict["mapmask"] + " MAPIN " + prefix + "_om.map3.tmp" +
